...
- **Unaddressed entries exist and ALL have a non-null `scope`** → this is a **FOCUSED ITERATION** on the union of those scopes:
  ...
  - Apply the feedback: run the three-loop strawman mechanic, the ADR-anchored decisions mechanic, and the raw→structured pass for the targeted instances only, going deeper on the touchpoint than a sweep would. Incorporate feedback by REWRITING the affected text in place — never by annotating the document with the feedback, a response to it, or inference about it (EDITORIAL DISCIPLINE, AI-PROBE-010).
  - Living SOAP: refresh ONLY the targeted PS's §6 blocks and the §6 status-summary counts — no full regeneration.
  - Checks: run per-instance auto/AI checks for the targeted instances only — including the per-instance §3-fit aspect of AI-PROBE-011 on the targeted PS. SKIP cross-instance checks (AI-PROBE-001, AI-PROBE-003, the cross-PS sameness aspect of AI-PROBE-011, cross-PS reconciliation) — they are only meaningful over the full set.
...
2. **For each open PS**, read the PS file + its corresponding notes file at `workspace/artifacts/transformation/notes/PS-{slug}-notes.md`. Apply the three-loop strawman mechanic from `probe-converge.yml` agent_guidance:
   - **Loop 1 (at PS open, once)**: draft the §3 Problem Analysis (focus areas the source capability's gap turns on — FIT, DON'T FILL) and the initial strawman from source capability + conceptual sketch + context + `initial-design` notes; populate §3, §4.1 (Shape), 4.2 (components), 4.3 (decisions)
   - **Loop 2 (each iteration)**: scan the §3 analysis AND the strawman for gaps (an unowned boundary or unvalidated cause in §3; components unspecified, data unsourced, decisions unstated, assumptions unvalidated in §4); propose candidate OQs with draft Question + draft Why/rationale
   - **Loop 3 (each iteration)**: when OQs resolve, propose strawman edits (and any §3 analysis refinements) in Format B (Existing / Proposed / Why with trace to evidence)
...
- **Hybrid AI checks** (`AI-PROBE-011 problem_analysis_fits`): a per-instance aspect (does THIS PS's §3 fit its problem?) run every iteration including focused, PLUS a cross-instance aspect (are the §3 focus areas uniform across all PS — filled-in, not analysed?) evaluated only on a full sweep.
